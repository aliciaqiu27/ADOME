...
//   var picturesId = JSON.stringify(response.data[i].relationships.pictures.data.id);
//   console.log(picturesId);
  
// }
// for (let i = 0; i < picturesId.length; i++) {
//   const element = picturesId.data[i].id;
// }

var myArray = [];
for(var i = 0, len = response.data.length; i < len; i++){
  myArray.push(response.data[i].relationships.pictures[2]);
}
...
for (let i = 0; i < response.included.length; i++) {
  ...
  // console.log(picture);
  const createdDate = response.included[i].attributes.created;
  // console.log(createdDate);

  // var data = response.data[i].relationships.pictures.data[i].id;
